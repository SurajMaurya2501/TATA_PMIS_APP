...
class _OAndMDashboardScreenState extends State<OAndMDashboardScreen> {
  ...
  void initState() {
    ...
    oAndMController.oAndMDashboardDatasource = OAndMDashboardDatasource(
        ...
        oAndMController.chargersMttrData,
        oAndMController.electricalMttrData,
        ...
        totalPendingFaults,
        totalFaultsResolved,
        oAndMController.chargerFaults,
        oAndMController.electricalFaults);
                  ...
                  children: [
                    Container(
                      ...
                      child: DropdownButtonHideUnderline(
                        ...
                        onChanged: (value) {
                          oAndMController.chargerFaults.clear();
                          oAndMController.electricalFaults.clear();
  ...
  Future getFaultsOccured() async {
    for (int i = 0; i < oAndMController.depotList.length; i++) {
      ...
      if (faultsQuery.exists) {
        ...
        List<String> sortElectricalList = [];
        for (Map<String, dynamic> map in data) {
          if (sortChargerList.contains(map['Equipment Name']) == false &&
              map['faultRelated'] == "Chargers") {
            sortChargerList.add(map['Equipment Name']);
          } else if (sortElectricalList.contains(map['Equipment Name']) ==
                  false &&
              map['faultRelated'] == "Electrical") {
            sortElectricalList.add(map['Equipment Name']);
          }
        }
        // faultsOccuredList.add(sortChargerList.length);
        oAndMController.chargerFaults.add(sortChargerList.length);
        oAndMController.electricalFaults.add(sortElectricalList.length);
        oAndMController.totalFaultOccured =
        ...
        oAndMController.chargerFaults.add(0);
        oAndMController.electricalFaults.add(0);
