...
class OAndMDashboardDatasource extends DataGridSource {
  ...
  OAndMDashboardDatasource(
      ...
      this.chargersMttrData,
      this.electricalMttrData,
  ...
  double totalMttrForChargers = 0.0;
  double totalMttrForElectrical = 0.0;
  List<DataGridRow> dataGridRows = [];
  final _dateFormatter = DateFormat.yMd();
  bool isTotalMttrConcluded = true;
  String totalChargerAvailabilityConclusion = "";
          ...
          : dataGridCell.columnName == 'chargers'
              ...
              : dataGridCell.columnName == 'buses'
                  ...
                  : dataGridCell.columnName == 'Depotname'
                      ...
                      : dataGridCell.columnName == 'chargerFaults'
                          ? dataGridRows.length - 1 == dataRowIndex
                              ? Container(
                                  alignment: Alignment.center,
                                  padding: const EdgeInsets.symmetric(
                          ...
                          : dataGridCell.columnName == 'electricalFaults'
                              ? dataGridRows.length - 1 == dataRowIndex
                                  ? Container(
                                      alignment: Alignment.center,
                                      padding: const EdgeInsets.symmetric(
                              ...
                              : dataGridCell.columnName == 'electricalMttr'
                                  ? dataGridRows.length - 1 == dataRowIndex
                                      ? Container(
                                          alignment: Alignment.center,
                                          padding: const EdgeInsets.symmetric(
  ...
  String getTotalChargerAvail() {
    totalChargerAvailabilityConclusion = '';
    double temp = 0.0;
    for (int i = 0; i < chargerAvailabilityList.length; i++) {
      temp = temp + double.parse(chargerAvailabilityList[i]);
    }
    return '${(temp / depotList.length).toStringAsFixed(2)}%';
  }

  String getAverageMttr(int dataRowIndex) {
    final calculaterMttr =
        chargersMttrData[dataRowIndex] / chargersFaults[dataRowIndex];
  ...
  String getTotalMttr(String columnName) {
    int totalFaults = 0;
    totalMttrForChargers = 0.0;
    totalMttrForElectrical = 0.0;
    if (columnName == "chargerMttr") {
      chargersFaults.every((element) {
        totalFaults = totalFaults + element;
        return true;
      });
      chargersMttrData.every((element) {
        totalMttrForChargers = totalMttrForChargers + element;
        return true;
      });
      return (totalMttrForChargers / totalFaults).toStringAsFixed(2);
    }
    if (columnName == "electricalMttr") {
      electricalFaults.every((element) {
        totalFaults = totalFaults + element;
        return true;
      });
      electricalMttrData.every((element) {
        totalMttrForElectrical = totalMttrForElectrical + element;
        return true;
      });
      return (totalMttrForElectrical / totalFaults).toStringAsFixed(2);
    }
  ...
  void onCellSubmit(DataGridRow dataGridRow, RowColumnIndex rowColumnIndex,
    ...
    } else if (column.columnName == 'chargerFaults') {
      dataGridRows[dataRowIndex].getCells()[rowColumnIndex.columnIndex] =
          DataGridCell<String>(
              columnName: 'chargerFaults', value: newCellValue);
      _dailyproject[dataRowIndex].chargersFaults = newCellValue;
    } else if (column.columnName == 'electricalFaults') {
      dataGridRows[dataRowIndex].getCells()[rowColumnIndex.columnIndex] =
          DataGridCell<String>(
              columnName: 'electricalFaults', value: newCellValue);
      _dailyproject[dataRowIndex].electricalFaults = newCellValue;
